...
router.get("/teams/:slug", allUsers, async (req: Request, res: Response) => {
	...
	const teams = await db
		...
		.leftJoin(OutwardDepartureAirport, eq(Teams.flightOutwardDepartureAirport, OutwardDepartureAirport.id))
		.leftJoin(OutwardLandingAirport, eq(Teams.flightOutwardLandingAirport, OutwardLandingAirport.id))
		.leftJoin(ReturnDepartureAirport, eq(Teams.flightOutwardLandingAirport, ReturnDepartureAirport.id))
	...
	if (req.user && req.user.isAdmin === true) {
		...
		res.send({ team, activities, tours, guides, airports, restaurants, vehicles, housings });
		return;
...
router.get("/teams/:slug/activities", async (req: Request, res: Response) => {
	const teams = await db
		.select({
			id: Teams.id,
			team: Teams.team,
...
router.get("/teams/:slug/tourists", auth, async (req: Request, res: Response) => {
	if (!req.user) {
	...
	if (teams.length > 0) {
		const tourists = await db
			...
			.leftJoin(TouristsPayments, and(eq(TouristsPayments.touristId, Tourists.id), eq(TouristsPayments.teamId, TouristTeams.teamId)))
		...
		if (req.user && req.user.isAdmin === true) {
			...
			res.send({ team, tourists, nationalities, genders, currencies, paymentMethods });
			return;
		} else {
...
router.get("/guides/:username", auth, async (req: Request, res: Response) => {
	if (!req.user) {
		res.send("You are not allowed to see this page");
		return;
	}
...
router.get("/teams", auth, async (req: Request, res: Response) => {
	...
	if (req.user.isAdmin === true) {
		...
		const teams = await db
			...
			.from(Teams)
			.leftJoin(Tours, eq(Teams.tourId, Tours.id))
			.where(and(sql`${today} BETWEEN DATE(${Teams.startsAt}) AND DATE(${Teams.endsAt})`, eq(Teams.guideId, req.user.id)));
...
router.get("/tourists/:id", auth, async (req: Request, res: Response) => {
